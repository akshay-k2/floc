...
for i in range(num_of_nodes):
    x_train[i] = np.load('even_np_datasets/dual_cluster_x_train' + str(i) + '.npy')
    y_train[i] = np.load('even_np_datasets/dual_cluster_y_train' + str(i) + '.npy')
...
def train_node(T, config, subset_idx):
    ...
    X = np.load('even_np_datasets/dual_cluster_x_train'+str(subset_idx)+'.npy')
    m = X.shape[0]
    n = X.shape[1] + 1
    X = np.concatenate((np.ones([m,1]),X), axis=1) 

    Y = np.load('even_np_datasets/dual_cluster_y_train'+str(subset_idx)+'.npy')
    cat = np.zeros([m,10])
